...
def wikify_old(_str):
    ...
    for _nt in en_notes:
        if _nt['title'] in _str:
            html_ref = '[' + _nt['title'] + '](' + _nt['link'] + ')'
            _str = _str.replace(_nt['title'], html_ref)
    return _str
...
def _new_wikis(_list, _nb):
    """Create new notes"""
    as_list = _applescriptify_list(_list)
    ...
    tell application id "com.evernote.Evernote"
        ...
        repeat with i from 1 to count of _list
            ...
            set matches to find notes "intitle:\\"" & _word & "\\" " & "notebook:\\"" & _nb & "\\""
            
            if matches = {{}} then
                set _note to create note title _word with text "" notebook _nb
        ...
        set new_notes to find notes "tag:wiki notebook:\\"" & _nb & "\\""
